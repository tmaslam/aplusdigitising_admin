...
PRODUCTION_DOMAIN="${PRODUCTION_DOMAIN:-aplusdigitizing.com}"
PRODUCTION_URL="${PRODUCTION_URL:-https://${PRODUCTION_DOMAIN}}"
PRODUCTION_SSH_HOST="${PRODUCTION_SSH_HOST:-68.65.121.228}"
PRODUCTION_SSH_USER="${PRODUCTION_SSH_USER:-digixjhl}"
PRODUCTION_SSH_PORT="${PRODUCTION_SSH_PORT:-21098}"
PRODUCTION_APP_PATH="${PRODUCTION_APP_PATH:-/home/digixjhl/aplusdigitizing_v2}"
...
This script packages the app as a zip, uploads it to production, preserves only:
  - .env
  - uploads
...
if [[ -z "${PRODUCTION_SSH_HOST}" || -z "${PRODUCTION_SSH_USER}" || -z "${PRODUCTION_APP_PATH}" ]]; then
...
REMOTE_TARGET="${PRODUCTION_SSH_USER}@${PRODUCTION_SSH_HOST}"
PACKAGE_NAME="production-deploy-$(date +%Y%m%d-%H%M%S).zip"
TMP_ROOT="$(mktemp -d)"
BUILD_ROOT="${TMP_ROOT}/build"
PACKAGE_PATH="${TMP_ROOT}/${PACKAGE_NAME}"
REMOTE_TMP_DIR="${PRODUCTION_APP_PATH}/.deploy-tmp-$$"
...
echo "  Target : ${REMOTE_TARGET}:${PRODUCTION_APP_PATH}"
echo "  URL    : ${PRODUCTION_URL}"
echo "  Port   : ${PRODUCTION_SSH_PORT}"
echo "  User   : ${PRODUCTION_SSH_USER}"
echo "  Domain : ${PRODUCTION_DOMAIN}"
echo
echo "This deploy will:"
echo "  1. Build a clean zip from the source app"
echo "  2. Upload it to the production server"
echo "  3. Preserve only .env and uploads inside the production app folder"
echo "  4. Delete everything else inside ${PRODUCTION_APP_PATH}"
...
if [[ "${ENABLE_VIEW_CACHE}" == "1" ]]; then
  ...
  echo "  8. Leave Blade uncached for safer frequent production deploys"
fi
echo

read -r -p "Type yes to continue with this production deployment: " CONFIRM_DEPLOY
...
if [[ "${HAS_SSHPASS}" == "1" ]]; then
  read -r -s -p "Production SSH password (leave blank to let ssh/scp prompt securely): " PRODUCTION_SSH_PASSWORD
  echo
else
  PRODUCTION_SSH_PASSWORD=""
...
if [[ -n "${PRODUCTION_SSH_PASSWORD}" ]] && [[ "${HAS_SSHPASS}" == "1" ]]; then
  SSH_CMD=(sshpass -p "${PRODUCTION_SSH_PASSWORD}" ssh -p "${PRODUCTION_SSH_PORT}" "${SSH_COMMON_OPTS[@]}")
  SCP_CMD=(sshpass -p "${PRODUCTION_SSH_PASSWORD}" scp -P "${PRODUCTION_SSH_PORT}" "${SSH_COMMON_OPTS[@]}")
else
  SSH_CMD=(ssh -p "${PRODUCTION_SSH_PORT}" "${SSH_COMMON_OPTS[@]}")
  SCP_CMD=(scp -P "${PRODUCTION_SSH_PORT}" "${SSH_COMMON_OPTS[@]}")
fi
...
if [[ "${DRY_RUN}" == "1" ]]; then
  ...
  echo "No files were transferred or changed on production."
  exit 0
fi

"${SSH_CMD[@]}" "${REMOTE_TARGET}" "mkdir -p '$(quote_for_single "${REMOTE_TMP_DIR}")' '$(quote_for_single "${PRODUCTION_APP_PATH}")'"
echo "Uploading package to production..."
"${SCP_CMD[@]}" "${PACKAGE_PATH}" "${REMOTE_TARGET}:${REMOTE_PACKAGE_PATH}"

REMOTE_SCRIPT=$(cat <<EOF
set -Eeuo pipefail

APP_PATH='$(quote_for_single "${PRODUCTION_APP_PATH}")'
...
if ! command -v unzip >/dev/null 2>&1; then
  echo "unzip is required on the production server." >&2
  exit 1
fi
...
if [ ! -f .env ] && [ -f .env.production.ready ]; then
  cp .env.production.ready .env
  echo "No existing .env found. Created .env from .env.production.ready."
fi
...
if grep -Eiq '^APP_DEBUG=(true|1)\$' .env; then
  echo "APP_DEBUG is enabled in production .env. Disable debug before deploying." >&2
  exit 1
fi

if ! grep -Eq '^DB_PASSWORD=.+$' .env; then
  echo "DB_PASSWORD is missing in .env. Set the production database password and rerun the deploy." >&2
  exit 1
fi
...
if command -v mysql >/dev/null 2>&1; then
  ...
  if [ -n "\${DB_HOST_VALUE}" ] && [ -n "\${DB_PORT_VALUE}" ] && [ -n "\${DB_NAME_VALUE}" ] && [ -n "\${DB_USER_VALUE}" ] && [ -n "\${DB_PASSWORD_VALUE}" ]; then
    ...
    if [ "\${SIGNUP_OFFER_COUNT}" = "0" ] || [ -z "\${SIGNUP_OFFER_COUNT}" ]; then
      ...
      echo "Run the production database rollout or insert the signup offer row before testing customer signup." >&2
      exit 1
    fi
  fi
fi
...
if [ "\${PAYMENT_PROVIDER_READY}" != "1" ]; then
  echo "2Checkout is not ready for production." >&2
...
if ! grep -Eq '^APP_KEY=.+$' .env; then
  echo "APP_KEY is missing. Generating a new application key for production..."
...
echo "Running remote deployment steps on production..."
