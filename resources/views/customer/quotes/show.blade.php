...
@section('content')
    <section class="content-card stack">
        <div class="section-head">
            ...
            <div style="display:flex; gap:12px; flex-wrap:wrap;">
                ...
                <form method="post" action="/aplus/quotes/{{ $order->order_id }}/delete" onsubmit="return confirm('Delete this quote?');">
                    @csrf
    ...
    <section class="content-card single-column" id="quote-response">
        ...
        @if ($showQuoteAcceptAction)
            <form method="post" action="/aplus/quotes/{{ $order->order_id }}/switch-to-order" class="form-grid">
                @csrf
                ...
                <div style="grid-column: 1 / -1; display:flex; gap:12px; flex-wrap:wrap; align-items:center;">
                    ...
                    @if ($showQuoteRejectAction)
                        ...
                        >Reject Quote</button>
                    @endif
                    <a class="button secondary" href="/aplus/view-quotes.php">Back To Quotes</a>
                </div>
            </form>
        @endif
        ...
        @if ($showQuoteRejectAction)
            ...
            <form
                method="post"
                action="/aplus/quotes/{{ $order->order_id }}/feedback"
                class="form-grid"
                data-reject-quote-panel
                @if (! ($errors->has('reason_code') || $errors->has('reason_text') || $errors->has('target_amount')))
                    hidden
        ...
        @if ($showQuoteSwitchEarly)
            ...
            <form method="post" action="/aplus/quotes/{{ $order->order_id }}/switch-to-order" onsubmit="return confirm('This quote has not been completed yet. Are you sure you want to switch it directly to an order?');">
                @csrf
